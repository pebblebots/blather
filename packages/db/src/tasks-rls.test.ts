...
describe.skipIf(!liveRlsTests)('tasks RLS policies', () => {
  ...
  it('enables RLS without forcing it and installs the intended policies', async () => {
    ...
    const policies = await query<{ tablename: string; policyname: string; cmd: string }>(sql, `
      SELECT tablename, policyname, cmd
      FROM pg_policies
      WHERE schemaname = 'public' AND tablename IN ('tasks', 'task_comments')
      ORDER BY tablename, policyname
    `);
    expect(policies).toEqual([
      { tablename: 'task_comments', policyname: 'task_comments_delete_authenticated_author', cmd: 'DELETE' },
      { tablename: 'task_comments', policyname: 'task_comments_insert_authenticated_visible', cmd: 'INSERT' },
      { tablename: 'task_comments', policyname: 'task_comments_select_authenticated_visible', cmd: 'SELECT' },
      { tablename: 'tasks', policyname: 'tasks_delete_authenticated_actor', cmd: 'DELETE' },
      { tablename: 'tasks', policyname: 'tasks_insert_authenticated_actor', cmd: 'INSERT' },
      { tablename: 'tasks', policyname: 'tasks_select_authenticated_relevant', cmd: 'SELECT' },
      { tablename: 'tasks', policyname: 'tasks_update_authenticated_actor', cmd: 'UPDATE' },
    ]);
  ...
  it('lets a channel participant read their channel tasks and the shared queue, but not another channel', async () => {
    const fixture = await seed();
    const rows = await asAuthenticated(fixture.viewerId, (tx) => query<{ id: string }>(tx,
      'SELECT id FROM tasks ORDER BY id',
    ));

    expect(rows.map((row) => row.id).sort()).toEqual([fixture.visibleTaskId, fixture.hiddenTaskId].sort());
  });
  ...
  it('does not expose or mutate comments attached to an unrelated task', async () => {
    const fixture = await seed();
    const commentId = randomUUID();
    await query(sql, `
      INSERT INTO task_comments (id, task_id, user_id, content)
      VALUES ($1, $2, $3, 'private task comment')
    `, [commentId, fixture.otherTaskId, fixture.outsiderId]);

    const comments = await asAuthenticated(fixture.viewerId, (tx) => query<{ id: string }>(tx,
    ...
    await expect(asAuthenticated(fixture.viewerId, (tx) => query(tx, `
      ...
      VALUES ($1, $2, 'unauthorized comment')
    `, [fixture.otherTaskId, fixture.viewerId]))).rejects.toThrow();

    const deleted = await asAuthenticated(fixture.viewerId, (tx) => query<{ id: string }>(tx,
  ...
  it('rejects deactivated users even when their pre-existing membership or actor row matches', async () => {
    const fixture = await seed();
    await query(sql, 'UPDATE users SET deactivated_at = NOW() WHERE id IN ($1, $2)', [
      fixture.viewerId,
      fixture.outsiderId,
    ]);

    const memberRows = await asAuthenticated(fixture.viewerId, (tx) => query<{ id: string }>(tx,
      'SELECT id FROM tasks WHERE id = $1',
      [fixture.visibleTaskId],
    ));
    expect(memberRows).toEqual([]);

    const actorUpdate = await asAuthenticated(fixture.outsiderId, (tx) => query<{ id: string }>(tx,
      "UPDATE tasks SET title = 'deactivated actor write' WHERE id = $1 RETURNING id",
      [fixture.visibleTaskId],
    ));
