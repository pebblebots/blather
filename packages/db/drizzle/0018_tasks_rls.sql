...
-- Postgres CI databases do not have Supabase's predefined role, while Supabase
-- does. Create the role only for local/vanilla Postgres compatibility.
DO $$
BEGIN
...
-- or server-maintained metadata. The API server uses the database owner and
-- therefore remains unaffected by these grants.
