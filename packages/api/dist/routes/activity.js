...
activityRoutes.get("/summary", async (c) => {
  ...
  let lines = [`## Activity since ${since}`];
  ...
  if (rows.length === 0) lines.push('- No activity recorded');

  return c.json({ summary: lines.join('\n'), rows });
});

// Helper: log activity (exported for use by other routes)
export async function logAgentActivity(db, { workspaceId, userId, action, targetChannelId, targetMessageId, metadata, sessionKey }) {
  try {
    await db.execute(sql`
      INSERT INTO agent_activity_log (workspace_id, agent_user_id, session_key, action, target_channel_id, target_message_id, metadata)
      VALUES (${workspaceId}, ${userId}, ${sessionKey || ''}, ${action}, ${targetChannelId || null}, ${targetMessageId || null}, ${JSON.stringify(metadata || {})}::jsonb)
    `);
  } catch (e) {
