...
channelRoutes.get('/:id/messages', async (c) => {
    ...
    if (around) {
        ...
        const seen = new Set();
        const deduped = allMsgs.filter(m => { if (seen.has(m.id))
        ...
        if (aroundMapped.length > 0) {
            ...
            const amReactions = await db.select().from(reactions)
                .where(sql `${reactions.messageId} IN (${sql.join(amIds.map((id) => sql `${id}`), sql `, `)})`);
            const amByMsg = {};
            for (const r of amReactions) {
                if (!amByMsg[r.messageId])
                    amByMsg[r.messageId] = [];
                amByMsg[r.messageId].push({ id: r.id, userId: r.userId, emoji: r.emoji, createdAt: r.createdAt });
            }
            for (const m of aroundMapped)
                m.reactions = amByMsg[m.id] || [];
        }
    ...
    if (result.length > 0) {
        ...
        const allReactions = await db.select().from(reactions)
            .where(sql `${reactions.messageId} IN (${sql.join(msgIds.map((id) => sql `${id}`), sql `, `)})`);
        const reactionsByMsg = {};
        for (const r of allReactions) {
            if (!reactionsByMsg[r.messageId])
                reactionsByMsg[r.messageId] = [];
...
channelRoutes.post('/:id/messages', async (c) => {
    ...
    if (msg.threadId) {
        const [replyCountResult] = await db.select({ count: sql `count(*)::int` }).from(messages).where(eq(messages.threadId, msg.threadId));
        await emitEvent(db, {
            workspaceId: channel.workspaceId,
            channelId,
            userId,
...
channelRoutes.post('/:id/read', async (c) => {
    ...
    const userId = c.get('userId');
    const channelId = c.req.param('id');
    await db.execute(sql `INSERT INTO channel_reads (channel_id, user_id, last_read_at)
...
channelRoutes.post('/:id/members', async (c) => {
    ...
    const [membership] = await db.select().from(channelMembers)
        ...
        .limit(1);
    if (!membership)
        return c.json({ error: 'You are not a member of this channel' }, 403);
...
channelRoutes.patch('/:id/archive', async (c) => {
    ...
    if (channel.isDefault)
        return c.json({ error: 'Cannot archive the default channel' }, 400);
...
channelRoutes.delete('/:channelId/messages/:messageId/reactions', async (c) => {
    ...
    const [existing] = await db.select().from(reactions)
        .where(and(eq(reactions.messageId, messageId), eq(reactions.userId, userId), eq(reactions.emoji, body.emoji))).limit(1);
    if (!existing)
        return c.json({ ok: true });
