  ...
  if (resend) {
    try {
      await resend.emails.send({
        ...
        html: `<p>Click the link below to log in to Blather:</p><p><a href="${magicUrl}">${magicUrl}</a></p><p>Or enter this code in the app: <strong>${code}</strong></p><p>This link expires in 15 minutes.</p>`,
      });
    } catch (err) {
